...
export const AssistantWidget: React.FC<AssistantWidgetProps> = ({
  ...
  // Show welcome tip only on Dashboard and Personal Time Study
  useEffect(() => {
  ...
  // Hide tip while the assistant popover is open
  useEffect(() => {
    ...
    <TooltipProvider delayDuration={200}>
      ...
      >
        ...
        <style>{`
          ...
          .animate-text-shine {
            background: linear-gradient(to right, #ffffff 20%, #c7d2fe 40%, #c7d2fe 60%, #ffffff 80%);
        ...
        `}</style>
        <div className="bg-[#6C5DD3] px-4 py-3 text-white flex items-center gap-2">
          <Info className="h-5 w-5 text-white/90" />
          <div>
            <h4 className="text-[16px] font-bold leading-none mb-1.5 tracking-wide drop-shadow-sm animate-text-shine">Page Assistant</h4>
            <p className="text-[12px] text-white/90 leading-none font-medium">Find answers to common questions</p>
          </div>
        </div>
