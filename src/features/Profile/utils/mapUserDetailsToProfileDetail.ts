...
export function mapUserDetailsToProfileDetailData(details: UserDetailsDto): ProfileDetailData {
  ...
  return {
    ...
    onRecords: {
      ...
      jobDutyStatement: details.documents?.find((d) => d.docType === "job_duty" && d.status === "active")?.fileName || "",
      jobDutyFileId: details.documents?.find((d) => d.docType === "job_duty" && d.status === "active")?.id || null,
