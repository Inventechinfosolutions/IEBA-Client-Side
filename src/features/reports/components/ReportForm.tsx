...
const reportSelectTrigger = cn(selectTriggerBase, "!h-12 w-full min-w-0 sm:min-w-[162.83px] px-[11px] !py-0")
...
const dateInputInRowClassName =
  "box-border h-12 w-full min-w-0 max-w-full sm:max-w-[168px] rounded-[8px] border border-[#d6d7dc] bg-white p-[9.29688px] text-[14px] text-[#111827] shadow-none focus-visible:border-[#6C5DD3] focus-visible:ring-1 focus-visible:ring-[#6C5DD3]/25"
...
const primaryReportButtonClass =
  "!h-[45px] !min-h-[45px] w-full sm:!w-[120px] shrink-0 rounded-[8px] !border-0 !bg-[#6C5DD3] !p-[4.46875px] !text-[14px] !font-medium !text-white hover:!bg-[#5b4fc2] focus-visible:!ring-2 focus-visible:!ring-[#6C5DD3]/35"


const stopReportButtonClass =
  "!h-[45px] !min-h-[45px] w-full sm:!w-[120px] shrink-0 rounded-[8px] !border-0 !bg-[#E73639] !p-[1.99219px] !text-[14px] !font-medium !text-white hover:!bg-[#cf2e31] focus-visible:!ring-2 focus-visible:!ring-[#E73639]/35"
...
/** Resolve inclusive FY calendar bounds (`YYYY-MM-DD`). Prefers API start/end; else Jul 1–Jun 30. */
function resolveFiscalYearDateBounds(
  fiscalYearId: string,
...
function defaultDatesWithinFiscalYear(bounds: { minDate: string; maxDate: string }): {
  from: string
  to: string
} {
  const now = new Date()
  const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`
  const from = clampIsoDateToRange(today, bounds.minDate, bounds.maxDate)
  const m = now.getMonth()
  const y = now.getFullYear()
  const qtrEndMonth = Math.floor(m / 3) * 3 + 2
  const qtrEndDate = new Date(y, qtrEndMonth + 1, 0)
  const toCandidate = `${y}-${String(qtrEndMonth + 1).padStart(2, "0")}-${String(qtrEndDate.getDate()).padStart(2, "0")}`
...
export function ReportForm({ module }: ReportFormProps) {
  ...
  const fiscalYearDateBounds = useMemo(() => {
    // Constrain pickers by FY when Fiscal Year is shown with Dates, Week, or Month mode.
    if (
      (selectMonthBy !== "dates" &&
        selectMonthBy !== "week" &&
        selectMonthBy !== "month") ||
    ...
    return resolveFiscalYearDateBounds(fiscalYearId, fiscalYearsData)
  }, [selectMonthBy, showTopLevelFiscalYear, fiscalYearId, fiscalYearsData])

  const fiscalYearMonthBounds = useMemo(() => {
    if (!fiscalYearDateBounds) return null
    return {
      minMonth: fiscalYearDateBounds.minDate.slice(0, 7),
      maxMonth: fiscalYearDateBounds.maxDate.slice(0, 7),
    }
  ...
  const clampDatesToFiscalYearBounds = useCallback(
    (nextFiscalYearId: string) => {
      ...
      const bounds = resolveFiscalYearDateBounds(nextFiscalYearId, fiscalYearsData)
      if (!bounds) return
      ...
      if (selectMonthBy === "dates" || selectMonthBy === "week") {
        ...
        const currentTo = getValues("dateTo")?.trim() ?? ""
        if (currentFrom) {
          const nextFrom = clampIsoDateToRange(currentFrom, bounds.minDate, bounds.maxDate)
          if (nextFrom !== currentFrom) {
            setValue("dateFrom", nextFrom, { shouldValidate: true })
          }
        }
        if (currentTo) {
          const fromAfterClamp = clampIsoDateToRange(
            getValues("dateFrom")?.trim() || currentFrom,
            bounds.minDate,
            bounds.maxDate,
          )
          let nextTo = clampIsoDateToRange(currentTo, bounds.minDate, bounds.maxDate)
          if (fromAfterClamp && nextTo < fromAfterClamp) nextTo = fromAfterClamp
          if (nextTo !== currentTo) {
  ...
  useEffect(() => {
    if (!fiscalYearDateBounds || !fiscalYearId) return
    clampDatesToFiscalYearBounds(fiscalYearId)
  }, [
    clampDatesToFiscalYearBounds,
    fiscalYearDateBounds,
    fiscalYearId,
    ...
    return (
      <>
        <div className="flex w-full shrink-0 flex-col gap-1.5 sm:w-auto sm:flex-row sm:items-end sm:gap-4">
          <span className={cn(labelClassName, "whitespace-nowrap mb-0 sm:mb-2")}>Select Month By</span>
          <Controller
            name="selectMonthBy"
            control={control}
            render={({ field }) => (
              <RadioGroup
                className="flex min-h-[45px] flex-wrap items-center gap-x-4 gap-y-2"
                value={field.value}
                onValueChange={(v) => {
                  ...
                  if (v === "dates") {
                    if (fiscalYearDateBounds) {
                      const { from, to } = defaultDatesWithinFiscalYear(fiscalYearDateBounds)
                      if (currentReportItem?.key === "DSSRPT1") {
                        ...
                        setValue(
                          ...
                          clampIsoDateToRange(
                            addDays(from, 27),
                            fiscalYearDateBounds.minDate,
                            fiscalYearDateBounds.maxDate,
                          ),
                          { shouldValidate: true },
                        )
                      } else {
        ...
        {selectMonthBy === "qtr" ? (
          <>
            {!showTopLevelFiscalYear && (
              <div className="w-full min-w-0 sm:w-[152px] shrink-0">
                  ...
                  render={({ field }) => (
                    <SingleSelectDropdown
                      ...
                      onChange={(val) => {
                        field.onChange(val)
                        clearPeriodDependentPicks()
                        clampDatesToFiscalYearBounds(val)
                      }}
                      onBlur={field.onBlur}
          ...
          <div className="w-full min-w-0 sm:w-[180px] shrink-0">
          ...
          <>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <ReportMonthPicker
                    ...
                    onBlur={field.onBlur}
                    minMonth={fiscalYearMonthBounds?.minMonth}
                    maxMonth={fiscalYearMonthBounds?.maxMonth}
                  />
                )}
              />
            ...
            {selectMonthBy === "week" ? (
              <>
                <div className="w-full min-w-0 sm:w-[168px] shrink-0">
          ...
          <>
            ...
            <div className="w-full min-w-0 sm:w-[350px] shrink-0">
                  ...
                  <ReportDatePicker
                    ...
                    onChange={(val) => {
                      ...
                      if (currentReportItem?.key === "DSSRPT1" && val) {
                        let nextTo = addDays(val, 27)
                        if (fiscalYearDateBounds) {
                          nextTo = clampIsoDateToRange(
                            nextTo,
                            fiscalYearDateBounds.minDate,
                            fiscalYearDateBounds.maxDate,
                          )
      ...
      <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="flex min-w-0 flex-wrap items-end gap-3 pb-0.5 sm:gap-4">
          {showReportsDepartmentField && (
            <div className="w-full min-w-0 sm:w-auto sm:min-w-[220px] sm:max-w-[350px] shrink-0">
          ...
          <div className="w-full min-w-0 sm:w-[260px] md:w-[280px] lg:w-[320px] shrink-0">
        ...
        <div className="min-w-0 w-full pb-0">
          {secondaryLayout === "dynamic" && currentReportItem?.criteria ? (
            <div className="grid min-w-0 grid-cols-1 gap-x-10 gap-y-8 lg:grid-cols-2 lg:gap-y-0">
              {(() => {
          ...
          <div className="flex w-full flex-wrap items-center justify-between gap-x-4 gap-y-3 sm:w-auto sm:justify-end">
            ...
            <div className="flex w-full min-w-0 items-center gap-2 sm:w-auto">
              <Button
                type="button"
                variant="default"
                className={cn(primaryReportButtonClass, "flex-1 sm:flex-initial")}
              ...
              <Button
                type="button"
                variant="default"
                className={cn(stopReportButtonClass, "flex-1 sm:flex-initial")}
                onClick={onStop}
              >
                Stop
              </Button>
        ...
        <div className="flex min-w-0 flex-wrap items-end gap-3 pt-6 sm:gap-4">
          ...
          <div className="w-full min-w-0 sm:flex-1 sm:min-w-[160px] lg:max-w-[350px]">
          ...
          <Button
            type="button"
            variant="default"
            className={cn(primaryReportButtonClass, "w-full sm:!w-[120px]")}
