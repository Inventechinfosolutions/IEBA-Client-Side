...
import {
  ...
  type P112DateGroup,
  type P112GroupedEmployee,
  type P112Record,
  type P112ReportPdfProps,
...
const COLUMNS = [
  ...
  { key: "travelTime", width: W.travelTime, label: "Tvl Time" },
] as const
...
function DateSection({ dateGroup }: { dateGroup: P112DateGroup }) {
  return (
    <View style={styles.dateSection}>
      ...
      {dateGroup.records.map((record, index) => (
        <RecordRow key={`${dateGroup.date}-${record.program}-${record.subactivity}-${index}`} record={record} />
      ))}
      ...
      <View style={styles.row}>
        ...
        <View style={[styles.dateTotalBox, { width: W.travelTime }]}>
          <Text style={[styles.rightText, styles.boldText]}>
            {formatReportTime(dateGroup.totalTravelTime)}
          </Text>
        </View>
      </View>
    </View>
...
function RecordRow({ record }: { record: P112Record }) {
  return (
    <View style={styles.row}>
      <EmptyCell width={W.employee} />
      ...
      <Cell width={W.travelTime} align="right">
...
function EmployeeTable({ employee }: { employee: P112GroupedEmployee }) {
  const grandTotals = getP112GrandTotals(employee)
...
function P112ReportDocument({
  ...
  if (employees.length === 0) {
    return (
      <Document>
        <P112ReportPage meta={meta} footerVariant={footerVariant} printedOn={printedOn}>
...
export async function generateP112ReportPdf(props: P112ReportPdfProps): Promise<Blob> {
  const printedOn = props.printedOn ?? formatPrintedOnLabel()
  const meta = await buildResolvedPdfMeta(
    { ...props.meta, reportCode: props.meta?.reportCode ?? "P112" },
