  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[calc(100%-2rem)] max-w-[800px] rounded-[4px] border border-[#e2e5ee] p-0 translate-y-[-50%] md:translate-y-[-80%] shadow-[0_12px_28px_rgba(17,24,39,0.16)]"
      >
        <DialogHeader className="px-4 md:px-8 pb-2 pt-6">
          <DialogTitle className="text-center text-[24px] font-medium text-[#111827]">
            {mode === "edit" ? "Edit To Do" : "Add To Do"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="relative px-4 md:px-12 pb-6 md:pb-12 pt-1">
          {isSubmitting && (
          ...
          {isEditMode && isFetching && !serverData ? (
            ...
            <>
                ...
                {isEditMode ? (
                  <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_270px] items-start gap-6 md:gap-10">
                    <div>
                      ...
                      <TitleCaseInput
                        ...
                        className="h-[46px] w-full max-w-[300px] rounded-[8px] border-[#dfe3ee] text-[12px] placeholder:text-[12px] placeholder:text-gray-400 disabled:pointer-events-auto disabled:cursor-not-allowed disabled:border-[0.8px]! disabled:border-[#cfd4dd]! disabled:bg-[#d2d4d9]/20! disabled:text-black! disabled:opacity-100 focus-visible:border-[#6C5DD3] focus-visible:ring-1 focus-visible:ring-[#6C5DD333]"
                      />
                    </div>
                    <div>
                      <label className="mb-2 block text-[12px] text-[#111827]">*Status</label>
                      <div className="flex min-h-[46px] flex-wrap items-center gap-4 py-1">
                  ...
                  <div>
                    ...
                    <TitleCaseInput
                      {...form.register("title")}
                      placeholder="Enter To Do Title"
                      className="h-[46px] w-full max-w-[282px] rounded-[8px] border-[#dfe3ee] text-[12px] placeholder:text-[12px] placeholder:text-gray-400 focus-visible:border-[#6C5DD3] focus-visible:ring-1 focus-visible:ring-[#6C5DD333]"
                    />
                  </div>
                )}
                <div>
              ...
              <div className="mt-8 flex items-center justify-end gap-3 sm:gap-5">
                <Button
                  type="submit"
